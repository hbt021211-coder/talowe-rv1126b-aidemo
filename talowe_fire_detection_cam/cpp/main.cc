...
int main(int argc, char **argv)
{
    ...
    image_buffer_t draw_image;
    memset(&src_image, 0, sizeof(image_buffer_t));
    memset(&draw_image, 0, sizeof(image_buffer_t));
...
// ====================== VideoWriter ======================
    ...
    std::string send_pipeline =
        ...
        "caps=video/x-raw,format=RGB,width=1920,height=1080,framerate=15/1 ! "
        "queue ! "
        "videoconvert ! "
    ...
    writer.open(send_pipeline, cv::CAP_GSTREAMER, 0, 15, cv::Size(1920, 1080), true);
    ...
    while(true){
        ...
        // cv::rotate(src_frame, src_frame, cv::ROTATE_90_CLOCKWISE);//ROTATE_90_COUNTERCLOCKWISE
        ...
        draw_image.height = src_frame.rows;
        draw_image.width = src_frame.cols;
        draw_image.width_stride = src_frame.step[0];
        draw_image.virt_addr = src_frame.data;
        draw_image.format = IMAGE_FORMAT_RGB888;
        draw_image.size = src_frame.total() * src_frame.elemSize();
        ...
        for (int i = 0; i < od_results.count; i++)
        {
            ...
            draw_rectangle(&draw_image, x1, y1, x2 - x1, y2 - y1, COLOR_BLUE, 3);
            sprintf(text, "%s %.1f%%", coco_cls_to_name(det_result->cls_id), det_result->prop * 100);
            draw_text(&draw_image, text, x1, y1 - 20, COLOR_GREEN, 10);
        }
        ...
        draw_text(&draw_image, fps_text, 20, 20, COLOR_GREEN, 12);
    ...
    if (draw_image.virt_addr != NULL)
    {

        free(draw_image.virt_addr);
